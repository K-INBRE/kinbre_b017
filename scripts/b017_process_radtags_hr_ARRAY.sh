...
#MSUB -o /home/b961k922/scratch/kinbre/b017_macdonald/report_files/b017_process_radtags_hr_$(JOBID)-${MOAB_JOBARRAYINDEX}.out
#MSUB -l walltime=6:00:00
#MSUB -q sixhour
#MSUB -j oe
#MSUB -N b017_process_radtags_hr
#MSUB -t [1-6]

echo "Sourcing the config file"
source /panfs/pfs.local/scratch/sjmac/b961k922/kinbre/b017_macdonald/scripts/b017_config.sh

ARRAY_INFILE=${SCRIPTS_DIR}/b017_hr_plates_ARRAY_input.txt

plate=$(sed -n "${MOAB_JOBARRAYINDEX}"p ${ARRAY_INFILE})
echo "Current file: ${plate}"

HR_BARCODES=${INFO_DIR}/hr_barcodes.txt
...
grep ${plate} ${HR_BARCODES} | cut -f 2,3 > ${INFO_DIR}/${PROJECT}_${plate}_barcodes.txt

echo "*** Processing radtags for plate ${plate}... "
process_radtags -p ${HR_DATA_RAW}/Sample_S3HR_${plate}/ \
                -o ${HR_DATA_RAW}/Sample_S3HR_${plate} \
